export default async function submitApplication(req, user) {
    if (req.sessionsEnabled) {
        ...
        if (Date.now() - req.session.lastApplication < 3600000) return "Thank you! Please wait before submitting another application.";
            ...
            // req.SendMail.send("wyattsushi@gmail.com", `New Volunteer Application for Love INC of Lewis County`, `
            req.SendMail.send("exec.director@loveincoflewiscounty.org", `New Volunteer Application for Love INC of Lewis County`, `
            ...
            if (newApplication.email) req.SendMail.send(newApplication.email, `Volunteer Application Sent to Love INC of Lewis County`, `
                <img src="https://www.loveincoflewiscounty.org/img/banner.jpg" alt="Love INC of Lewis County Logo" style="width: 90%; height: auto;">
    ...
    else return "Error: A secure connection is required to submit an application.";
}