export default async function submitMessage(req, user) {
    if (req.sessionsEnabled) {
        ...
        if (Date.now() - req.session.lastMessage < 60000) return "Please wait before submitting another message.";
            ...
            // req.SendMail.send("wyattsushi@gmail.com", `New Message for Love INC of Lewis County`, `
            req.SendMail.send("exec.director@loveincoflewiscounty.org", `New Message for Love INC of Lewis County`, `
            ...
            req.SendMail.send(newMessage.email, `Message Sent to Love INC of Lewis County`, `
                <img src="https://www.loveincoflewiscounty.org/img/banner.webp" alt="Love INC of Lewis County Logo" style="width:90%; height:auto;">
    ...
    else return "Error: A secure connection is required to submit a message.";
}