class BibleViewer extends Container {
    constructor(settings = {}) {
        ...
        // books, chapters and verse amounts in each chapter
        this.bibleInfo = {
            "Genesis": [
    ...
    async render() {
        ...
        this.goButton = this.ui.addChild(new Button("go", async () => { 
            ...
            if (this.settings.bookmark) await base.do("save-bible-location", { 
                route: "/scripts", 
                bookmark: this.settings.bookmark, 
        ...
        if (this.settings.bookmark) await base.load(`last-notherBible-location-${this.settings.bookmark}`, "local").then((location) => {
            if (location[0]?.data) this.location = {...this.location, ...location[0].data};
        });      
    ...
    setBooks = () => {
        let books = Object.keys(this.bibleInfo);
