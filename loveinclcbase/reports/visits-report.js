    ...
    if (req.body.email && typeof req.body.email === "string" && req.body.email.includes("@")) {
        ...
        if (spirit) {        
            ...
            if (!spirit.data.tally) spirit.data.tally = {};
                ...
                for (let key in spirit.data.tally) {
                    // normalize the data
                    spirit.data.tally[key] = {
                    ...
                    if (spirit.data.tally[key].month === month && spirit.data.tally[key].year === year) {
                        thisMonthCount++;
                        thisMonthTime += spirit.data.tally[key].time;
                        relevant = true;
                    }
                    if (spirit.data.tally[key].lastMonth.month === month && spirit.data.tally[key].lastMonth.year === year) {
                        thisMonthCount++;
                        thisMonthTime += spirit.data.tally[key].lastMonth.time;
                        relevant = true;
                    }
                    // if the session is from last month, add the time to last month's total
                    if ((spirit.data.tally[key].month === month - 1 && spirit.data.tally[key].year === year) ||
                        (spirit.data.tally[key].month === 11 && spirit.data.tally[key].year === year - 1 && month === 0)) {
                        lastMonthCount++;
                        lastMonthTime += spirit.data.tally[key].time;
                        relevant = true;
                    }
                    if ((spirit.data.tally[key].lastMonth.month === month - 1 && spirit.data.tally[key].lastMonth.year === year) ||
                        (spirit.data.tally[key].lastMonth.month === 11 && spirit.data.tally[key].lastMonth.year === year - 1 && month === 0)) {
                        lastMonthCount++;
                        lastMonthTime += spirit.data.tally[key].lastMonth.time;
                        relevant = true;
                    }

                    if (!relevant) delete spirit.data.tally[key];
                }
            }
    
            await req.SendMail.send(req.body.email, `Love INC of Lewis County Visits Report`, `
                ...
                <br>
                <br>
                <p>Visitors counts and time spent are estimated by tracking cookies and will result in some overcount.</p>
