export default async (req, user) => {
    if (!req.body.name || !req.body.location || !req.body.payment) {
        ...
        let spirit = await req.Spirit.create("shirt-orders", {}, null);
        await spirit.commit({
            name: req.body.name,
        ...
        if (req.body.email && typeof req.body.email === "string" && req.body.email.includes("@")) await req.SendMail.send(req.body.email, `Thank you for your order!`, `
            <h1>Nother Shirts</h1> <br>
            Order ID: ${spirit._id} <br><br>
            <p>Thank you for ordering from Nother Shirts! Please keep an eye on the page linked below for updates on your order:</p> <br>
            <a href="https://notherbase.com/shirts?o=${spirit._id}">https://notherbase.com/shirts?o=${spirit._id}</a> <br><br>
        ...
        return { newID: spirit._id, message: `Order created! Thank you!` };
    }
}